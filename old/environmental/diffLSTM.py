...
def time_series_analysis_plot(species_series, dataframe_complete, filename, bacteria):
    ...
    mycolors = np.random.choice(
        list(mpl.colors.XKCD_COLORS.keys()), len(bacteria), replace=True
    )
...
def plot_loss(history):
    ...
    figure.savefig(
        "Milwaukee/LSTM_new/loss_withMetadata.png",
...
complete, num_taxa, dic_taxa = create_complete_df(
    "/local/work/16S/snakemake_qiime/16S/MachineLearning/Wastewater/Dinslaken/timeseries.tsv",
    "/local/work/16S/snakemake_qiime/16S/MachineLearning/Wastewater/Dinslaken/index_only.tsv",
    "/local/work/16S/snakemake_qiime/16S/MachineLearning/Wastewater/Dinslaken/metadata.tsv",
)
# print(dic_taxa)
...
ensemble_list = ensemble.fit_ensemble(
    4, X_train, X_val, X_test, y_train, y_val, scaler, num_taxa
)
print("Ensemble generated!")
prediction_inter = ensemble.predict_with_pi(
    ensemble_list, X_train, X_test, y_test, scaler, num_taxa
)
...
predictions_reshaped = np.concatenate(
    (predictions_reshaped, X_test.reshape(X_test.shape[0], X_test.shape[2])), axis=1
)
# print(predictions_reshaped.shape)
predictions_reshaped = scaler.inverse_transform(predictions_reshaped)[:, 0:num_taxa]
y_test = scaler.inverse_transform(
    np.concatenate([y_test, X_test.reshape(X_test.shape[0], X_test.shape[2])], axis=1)
)[:, 0:num_taxa]
...
val_pred_reshaped = np.concatenate(
    (val_pred_reshaped, X_val.reshape(X_val.shape[0], X_val.shape[2])), axis=1
)
...
train_pred_reshaped = np.concatenate(
    (train_pred_reshaped, X_train.reshape(X_train.shape[0], X_train.shape[2])), axis=1
)
train_pred_reshaped = scaler.inverse_transform(train_pred_reshaped)[:, 0:num_taxa]

y_train = scaler.inverse_transform(
    np.concatenate(
        [y_train, X_train.reshape(X_train.shape[0], X_train.shape[2])], axis=1
    )
)[:, 0:num_taxa]
...
for i in range(num_taxa):
    ...
    plt.plot(
        complete.iloc[:train_size]["Time"], train_pred_reshaped[:, i], label="Training"
    )
    plt.plot(
        complete.iloc[train_size:val_size]["Time"],
        val_pred_reshaped[:, i],
        label="Validation",
    )
    plt.plot(
        complete.iloc[val_size:]["Time"],
        prediction_inter[i][0][1 : len(prediction_inter[i][0])],
        label="upper",
    )
    plt.plot(
        complete.iloc[val_size:]["Time"],
        prediction_inter[i][1][1 : len(prediction_inter[i][1])],
        label="lower",
    )
    plt.plot(
        complete.iloc[val_size:]["Time"],
        prediction_inter[i][2][1 : len(prediction_inter[i][2])],
        label="mean",
    )
...
def retrain_model(timeseries, taxalist, exo, model, scaler):
    ...
    for y in range(num_taxa):
        for i in range(1, 4):
            new_timeseries[f"Target{y+1}_Lag_{i}"] = new_timeseries[
                f"Target{y+1}"
            ].shift(i)
            lag_targets += 1
    ...
    y_test = scaler.inverse_transform(
        np.concatenate(
            [y_test, X_test.reshape(X_test.shape[0], X_test.shape[2])], axis=1
        )
    )[:, 0:num_taxa]
    ...
    # train_pred_reshaped = np.concatenate((train_pred_reshaped,X_val.reshape(X_val.shape[0],X_val.shape[2])), axis=1)
    train_pred_reshaped = np.concatenate(
        (train_pred_reshaped, X_train.reshape(X_train.shape[0], X_train.shape[2])),
        axis=1,
    )
    train_pred_reshaped = scaler.inverse_transform(train_pred_reshaped)[:, 0:num_taxa]
    ...
    error = errorInterval.predict_with_pi(
        predictions_reshaped, X_train, num_taxa, nrmse
    )
    ...
    for i in range(num_taxa):
        # Visualize predictions for Targets
        plt.plot(
            new_timeseries["Time"], new_timeseries[f"Target{i+1}"], label="True Values"
        )
        # plt.plot(complete.iloc[val_size:]['Time'], predictions_reshaped[:, i], label='Predictions test')
        plt.plot(
            new_timeseries.iloc[:train_size]["Time"],
            train_pred_reshaped[:, i],
            label="Training",
        )
        plt.plot(
            new_timeseries.iloc[train_size:val_size]["Time"],
            val_pred_reshaped[:, i],
            label="Validation",
        )
        plt.plot(
            new_timeseries.iloc[val_size:]["Time"],
            error[i][0][1 : len(error[i][0])],
            label="upper",
        )
        plt.plot(
            new_timeseries.iloc[val_size:]["Time"],
            error[i][1][1 : len(error[i][1])],
            label="lower",
        )
        # plt.plot(complete.iloc[val_size:]['Time'],prediction_inter[i][2][1:len(prediction_inter[i][2])], label = "mean")
        plt.plot(
            new_timeseries.iloc[val_size:]["Time"],
            predictions_reshaped[:, i],
            label="Test",
        )
        plt.fill_between(
            new_timeseries.iloc[val_size:]["Time"],
