...
def plot_loss(history):
    ...
    figure.savefig(
        "Dinslaken/GRU/loss.png", bbox_extra_artists=(lgd,), bbox_inches="tight"
    )
    plt.clf()
...
model.add(
    GRU(units=1024, activation="relu", input_shape=(X_train.shape[1], X_train.shape[2]))
)
model.add(Dropout(0.2))
...
shap_plots.shap_featureimportance_plots(
    model,
    X_train,
    y_train,
    X_test,
    dic_taxa,
    complete_woT.columns[num_taxa : (num_taxa + lag_targets + 2)],
)

ensemble_list = ensemble.fit_ensemble(
    5, X_train, X_val, X_test, y_train, y_val, scaler, num_taxa
)
