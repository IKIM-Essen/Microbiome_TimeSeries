...
def shap_featureimportance_plots(model, Xtrain, ytrain, Xtest, taxa_dict, columns):
    ...
    print(Xtrain.shape)
    # print(type(species))
    shap_values_2D = shap_values[0].reshape(
        shap_values[0].shape[0], shap_values[0].shape[2]
    )
    ...
    shap.initjs()
    shap.summary_plot(
        shap_values_2D, X_train_2D, feature_names=new_list, plot_type="bar", show=False
    )
    plt.savefig(plotpath + "shap_wM.png", bbox_inches="tight")

    plt.clf()
    shap.summary_plot(shap_values_2D, X_train_2D, feature_names=new_list, show=False)
    ...
    shap.plots._waterfall.waterfall_legacy(
        DE.expected_value[0], shap_values[0][0][0], feature_names=new_list
    )
