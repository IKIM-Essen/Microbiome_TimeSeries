...
def retrain_model(timeseries, taxalist, exo, tcn_path, lstm_path, scaler, output_path, predictions_out, anomalies_path, train_percentage, val_percentage):
    ...
    X_train, y_train, X_val, y_val, X_test, y_test = split_without_scaling(scaled_data, num_taxa, train_percentage, val_percentage, split_sizes_path="results/intermediate/split_sizes.pkl")
    ...
    # --- Compute residuals ---
    predictions_train, predictions_val, predictions_test, tcn, lstm = predict_retrain(X_train, y_train, X_val, y_val, X_test, tcn, lstm, output_path)
