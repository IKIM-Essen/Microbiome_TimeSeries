...
LOG_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "logs", "predicting"))
...
def mae(actual, predicted):
    ...
    logger.info("MAE computed: %f | actual_inv shape=%s | pred_inv shape=%s", value, getattr(actual_inv, 'shape', None), getattr(inverse, 'shape', None))
    return value
...
def combine_metrics(y_train, y_test, predict_train, predict_test, output_path=None):
    ...
    if output_path is not None:
        ...
        logger.info("Saved metrics to TSV: %s | columns=%s", output_path, list(df.columns))
    return metrics