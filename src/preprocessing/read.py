...
def preprocess_metadata(metadata):
    ...
    logger.info("Preprocessing metadata: season_map keys=%s, month_map keys=%s, drop_cols=%s",
                list(season_map.keys()), list(month_map.keys()), cols_to_drop)
    ...
    metadata_woT = metadata_woT.drop(columns=[c for c in cols_to_drop if c in metadata_woT.columns])
...
def read_timeseries(path_taxa, path_data):
    ...
    data_df_reduced.index = (
        data_df_reduced.index
        .str.split("; s__")
        .str[0]
    )
    data_df_reduced = data_df_reduced[
        ~data_df_reduced.index.str.contains("Unassigned")]
...
def create_complete_df(path_prob,path_exo,path_tax,include_metadata = False,output="results/output.csv"):
    """
    ...
    """
    logger.info("Creating complete dataframe from prob=%s exo=%s tax=%s include_metadata=%s output=%s",
                path_prob, path_exo, path_tax, include_metadata, output)
