...
def split_data(
    ...
    window_length = 3
    for split_name, split_array in (
        ("X_train", X_train),
        ("X_val", X_val),
        ("X_test", X_test),
    ):
        n_features = split_array.shape[1]
        if n_features % window_length != 0:
            pad_width = window_length - (n_features % window_length)
            split_array = np.pad(
                split_array,
                ((0, 0), (0, pad_width)),
                mode="constant",
