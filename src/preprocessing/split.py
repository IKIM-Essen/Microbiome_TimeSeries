...
def split_without_scaling(scaled_data, num_taxa, train_percentage, val_percentage, split_sizes_path="results/intermediate/split_sizes.pkl"):
    logger.info("Splitting data for %s taxa", num_taxa)
    
    count = sum(not col.startswith("Target") and not col.startswith("Time") for col in scaled_data.columns)
    print(count)
    X = scaled_data.reshape(scaled_data.shape[0], scaled_data.shape[1])  # Reshape for LSTM input
    y = scaled_data[:, 0:num_taxa+count]
    # Split the data into training and testing sets
