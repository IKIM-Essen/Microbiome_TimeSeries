...
bacteria, df = general_functions.merge_dataframes("bacteria_list_final4.tsv", "../table.from_biom_w_taxonomy-featcount_femalegut.txt")
df_trans, species, df_reduced_second = general_functions.read_csv(df)

ml_plots.time_series_analysis_plot(species, df_trans, plotpath+"plot_original.png", bacteria)

#interpolate_df = general_functions.interpolate_samples(df_trans,1)
ml_plots.time_series_analysis_plot(species, df_trans, plotpath+"plot_interpolated.png", bacteria)
...
model, history = general_functions.create_model("adam", "mae", n_steps, n_features, Xtrain, Ytrain, Xval, Yval)
print(history.history)
# Save model
...
n_members = 15
ensemble = model_confidence.fit_save_ensemble(n_members, Xtrain, Xval, Ytrain, Yval, scaler)
# make predictions with prediction interval
yhat_list, error_hat = model_confidence.predict_with_pi(ensemble, Xtest, scaler.inverse_transform(Ytest), scaler, species, n_steps)

present_orig, present_predicted = get_only_predicted_bacteria("bacteria_list_final4.tsv", bacteria, testY, predictTest)
...
ml_plots.confidence_plot(plotpath+"confidence_plots/", species, predictTest, yhat_list, n_steps)

ml_plots.prediction_plot(plotpath+"prediction_plots/", df_trans, predictX, predictval, predictTest, species, yhat_list, error_hat, n_steps, eval_dict, outlier)
...
general_functions.get_metrics(trainY,predictX,valY,predictval,present_orig,array_list_small,plotpath+"errormetrics.csv")