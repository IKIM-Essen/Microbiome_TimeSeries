...
def model_sklearn(Xtrain, Ytrain, Xval, Yval, scaler, optimizer, loss, n_features, n_steps):
    """
    Creating a sklearn model out of a keras model
    """
    model_sklearn = KerasRegressor(build_fn=create_model(optimizer, loss, n_steps, n_features, Xtrain, Ytrain, Xval, Yval), epochs=40, batch_size=50, verbose=2)
    tscv = TimeSeriesSplit(n_splits=5)
    score = cross_val_score(model_sklearn, Xtrain, Ytrain, cv = tscv, scoring = "neg_mean_absolute_error")
    ...
    grid_result = grid.fit(Xtrain, Ytrain,  validation_data=(Xval, Yval), epochs=400, callbacks = [es])
