...
complete, num_taxa = create_complete_df(
    ...
    "/local/work/16S/snakemake_qiime/16S/MachineLearning/HostMicrobiome/bacteria_list_final4.tsv",
...
# print(predictions_reshaped.shape)
# print(X_test.shape)
lstm_predictions_reshaped = np.concatenate(
    (lstm_predictions_reshaped, X_test_L.reshape(X_test_L.shape[0], X_test_L.shape[2])),
    axis=1,
)
# print(predictions_reshaped)
lstm_predictions_reshaped = scaler.inverse_transform(lstm_predictions_reshaped)[
    :, 0:num_taxa
]

rf_predictions = scaler.inverse_transform(
    np.concatenate((predictions_RF, X_test_RF), axis=1)
)[:, 0:num_taxa]

ensemble_predictions = scaler.inverse_transform(
    np.concatenate((ensemble_predictions, X_test_RF), axis=1)
)[:, 0:num_taxa]

y_actual = scaler.inverse_transform(
    np.concatenate(
        (y_test_L, X_test_L.reshape(X_test_L.shape[0], X_test_L.shape[2])), axis=1
    )
)[:, 0:num_taxa]
...
ensemble_predictions_train = scaler.inverse_transform(
    np.concatenate((ensemble_predictions_train, X_train_RF), axis=1)
)[:, 0:num_taxa]
y_train_actual = scaler.inverse_transform(
    np.concatenate(
        (y_train_L, X_train_L.reshape(X_train_L.shape[0], X_train_L.shape[2])), axis=1
    )
)[:, 0:num_taxa]
